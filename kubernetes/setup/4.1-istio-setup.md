...
```
`Note: there should be only

one Gateway in the whole cluster.`
...
### Enable Side Car
```
kubectl label namespace ekl-dev istio-injection=enabled --overwrite

kubectl get ns ekl-dev --show-labels
```
